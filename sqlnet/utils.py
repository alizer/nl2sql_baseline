...
def load_dataset(toy=False, use_small=False, mode='train'):
    ...
    dev_sql, dev_table = load_data(
        'data/val/val.json', 'data/val/val.tables.json', use_small=use_small)
    dev_db = 'data/val/val.db'
    if mode == 'train':
        train_sql, train_table = load_data(
            'data/train/train.json', 'data/train/train.tables.json', use_small=use_small)
        train_db = 'data/train/train.db'
        return train_sql, train_table, train_db, dev_sql, dev_table, dev_db
    elif mode == 'test':
        test_sql, test_table = load_data(
            'data/test/test.json', 'data/test/test.tables.json', use_small=use_small)
...
def to_batch_seq(sql_data, table_data, idxes, st, ed, ret_vis_data=False):
    ...
    for i in range(st, ed):
        ...
        vis_seq.append((sql['question'],
                        table_data[sql['table_id']]['header']))
    if ret_vis_data:
    ...
    for st in tqdm(range(len(sql_data) // batch_size + 1)):
        ...
        try:
            ...
            one_err, tot_err = model.check_acc(
                raw_data, pred_queries, query_gt)
    ...
    with open(file_name) as inf:
        for idx, line in enumerate(inf):
            ...
            if info[0].lower() not in ret:
                ret[info[0].decode(
                    'utf-8')] = np.array(map(lambda x: float(x), info[1:]))
    return ret
