...
class DBEngine:
    ...
    def execute(self, table_id, select_index, aggregation_index,
                conditions, condition_relation):
        """
        ...
        if condition_relation == 0 and len(conditions) > 1:
            return 'Error1'
        # 选择列或条件列为0
        if len(select_index) == 0 or len(
                conditions) == 0 or len(aggregation_index) == 0:
            return 'Error2'
        ...
        for col_index, op, val in conditions:
            if isinstance(val, str):
                ...
                where_part.append(
                    'col_{} {} "{}"'.format(
                        col_index + 1, cond_op_dict[op], val))
        where_part = 'WHERE ' + condition_relation.join(where_part)

        query = 'SELECT {} FROM {} {}'.format(
            select_part, table_id, where_part)
